...
├── packages/
│   ├── core/                       # Auth logic, DB client, shared types, context registry, merge-schemas script
...
Every module exports a typed manifest that declares its identity, what context it exposes, what it consumes, and what tier of interaction it supports:
...
- AI-powered suggestions (Context Registry and Tier 2 manifest are in place as the hook)
