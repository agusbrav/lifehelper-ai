...
export function AnalyticsMonthNav({ availableMonths, selectedYear, selectedMonth }: Props) {
  ...
  // availableMonths sorted newest first
  ...
  const btnCls = 'border border-[var(--border)] rounded-lg px-3 py-1 text-sm text-[var(--muted-fg)] hover:text-[var(--fg)] hover:bg-[var(--accent-muted)] transition-colors disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-[var(--muted-fg)]'

  return (
    <div className="flex items-center gap-2">
      <button onClick={() => navigate(-1)} disabled={isOldest} className={btnCls}>←</button>
      <span className="text-base font-semibold text-[var(--fg)] min-w-[180px] text-center capitalize">
        {monthLabel}
      </span>
      <button onClick={() => navigate(1)} disabled={isNewest} className={btnCls}>→</button>
      {!isCurrentMonth && (
        <button
          onClick={() => router.push('/m/budget/analytics')}
          className={`ml-1 ${btnCls} text-xs`}
        >
          {t('today')}
        </button>
      )}
